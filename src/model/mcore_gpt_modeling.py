...
class ShardedGptLogit(torch.nn.Module):
    def __init__(self, vocab_size, hidden_size, world_size):
        ...
        self.embedding_weight = nn.Embedding(
            vocab_size // world_size, hidden_size
        ).weight
...
class ShardedGptSelfAttention(torch.nn.Module):
    def __init__(
        ...
        self.query_key_value = ColumnParallelLinear(
            hidden_size, 3 * hidden_size, world_size, gather_output=False
        )
        ...
        self.scale_mask_softmax = FusedScaleMaskSoftmax(
            True, False, masked_softmax_fusion, attention_mask_func, True, coeff
        )
...
class ShardedGptTransformerLayer(torch.nn.Module):
    def __init__(
        ...
        self.mlp = ShardedGptMlp(hidden_size, world_size, output_dropout_prob)
    ...
    def forward(self, hidden_states, ltor_mask):
        ...
        output = bias_dropout_add_func(
            mlp_output, mlp_bias.expand_as(layernorm_input), layernorm_input, 0.1
        )
...
# Original code might be useful because I think it implement not TE version of transformer.
##############################################################################################
class CustomGPTTransformerLayer(torch.nn.Module):
    def __init__(
        self,
        hidden_size,
        world_size,
        ...
        super(CustomGPTTransformerLayer, self).__init__()
...
class CustomGPTTransformer(torch.nn.Module):
    def __init__(
        self,
        num_layers,
        hidden_size,
        ...
        super(CustomGPTTransformer, self).__init__()
